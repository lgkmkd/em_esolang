import time, ctypes, os, sys, socket
...
class KERNEL:
	def call(self, processor):
		...
		elif(self.eax == 7): # system time
			processor.mem.registers['EAX'] = int(time.time())
		
		elif(self.eax == 8): # sleep
			time.sleep(self.ebx)
		
		elif(self.eax == 9): # mkdir
			try:
				os.mkdir(self.de)
			except FileExistsError:
				pass
		
		elif(self.eax == 10): # delete file
			try:
				os.remove(self.de)
			except FileNotFoundError:
				pass
		
		elif(self.eax == 11): # delete folder
			try:
				os.rmdir(self.de)
			except OSError:
				pass
